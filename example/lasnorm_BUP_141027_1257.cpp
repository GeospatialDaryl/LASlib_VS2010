  ...
  */
  ...
  while (lasreader->read_point())
  {
	...
	GDALRasterIO( hBand, GF_Read, iPixel, iLine, 1, 1, 
					  adfPixel, 1, 1, GDT_Float32, 0, 0) ;
	
	adfPixel[0] = adfPixel[0] * dfScale + dfOffset;
	adfPixel[1] = adfPixel[1] * dfScale + dfOffset;
	adfPixel[1] = adfPixel[1] * dfScale + dfOffset;
	fprintf(stdout, "This val oupt2 %10.3d \n", adfPixel );
	fprintf(stdout, "This val oupt3 %18.7d \n", adfPixel[1] );
	fprintf(stdout, "%.15g  \n" , adfPixel[0]);

	fprintf(stdout, "This val  (fprintf)  %8.2f  %8.2f  %8.2f  %8.2f \n", adfPixel[0] ,adfPixel[1] ,adfPixel2[0] ,adfPixel2[1] );
	
    lasreader->point.Z = (dfGeoZ - adfPixel2[0]) * 100.;
    //fprintf (stdout," X,Y,Z,demZ :  %18.2f  %18.2f  %10.2f %10.2f \n", dfGeoX, dfGeoY, dfGeoZ, adfPixel2[0]);
